        ...
        for f=1:n
            ...
            while alphacol[f] > params.min_stepsize
                ...
                if new_obj_by_col < obj_by_col[f]
                    copyto!(vf[f], newvf[f])
                    alphacol[f] *= stepsize_incr
                    obj_by_col[f] = new_obj_by_col
                    break
                else
                    copyto!(newvf[f], vf[f])
                    alphacol[f] *= stepsize_decr
...
# STEP 4: Check stopping criterion
        ...
        if i>10 && (obj_decrease < scaled_abs_tol || obj_decrease/obj < params.rel_tol)
            if scaled_abs_tol < 0 || obj_decrease/obj < 0
                println("###########################")
                println("###########################")
                println("###########################")
                println(scaled_abs_tol)
                println("OBJ - 1: ", ch.objective[end-1])
                println("OBJ:     ", ch.objective[end])
            end
            print_iteration_info(i, ch.objective[end], scaled_abs_tol, params.rel_tol, obj_decrease)
            break
        end
...
function print_iteration_info(i, obj, abs_tol, rel_tol, obj_decrease)
    println("ITERATION $i:,  obj value = $(round(obj, digits=7)) ||| ",  "rel_tol = $(rel_tol) ||| ", "abs_tol = $(abs_tol)")
